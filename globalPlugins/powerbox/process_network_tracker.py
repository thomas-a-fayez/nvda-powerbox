# -*- coding: utf-8 -*-
# process_network_tracker.py - Real-Time Process Network Sockets & Security Tracker for PowerBox

# Acknowledgment:
# - Low-level TCP/UDP socket enumeration utilizes Microsoft IP Helper APIs (GetExtendedTcpTable, GetExtendedUdpTable).
# - Individual socket termination utilizes SetTcpEntry with MIB_TCP_STATE_DELETE_TCB.
# - Clean Three-Tier Tabbed Architecture (Internet, Local LAN, Localhost/Listeners) separates traffic domains cleanly.
...
# --- Accessible Real-Time Process Network Tracker Dialog (3-Tier Tabbed Console) ---
class ProcessNetworkTrackerDialog(wx.Dialog):

    TAB_INTERNET = 0
    TAB_LAN = 1
    TAB_LOCAL = 2

    def __init__(self, parent, app_name, pids):
        super(ProcessNetworkTrackerDialog, self).__init__(
            parent,
            title=_("Network Connections - {app} ({count} PIDs)").format(app=app_name, count=len(pids)),
            style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER,
        )

        self.app_name = app_name
        self.pids = set(pids)
        self.connection_history = {}  # Unique key -> socket metadata

        # Responsive Dialog Sizing (78% of active display area)
        display_rect = wx.Display().GetClientArea()
        w = max(980, int(display_rect.width * 0.78))
        h = max(620, int(display_rect.height * 0.78))
        self.SetSize((w, h))
        ...
        # 2. Accessible Notebook (Tabs: Internet, Local LAN, Localhost/Listeners)
        ...
        for i, name in enumerate(tab_names):
            ...
            list_ctrl.InsertColumn(0, _("Remote IP"))
            list_ctrl.InsertColumn(1, _("Host / Domain"))
            list_ctrl.InsertColumn(2, _("Port / Service"))
            list_ctrl.InsertColumn(3, _("Protocol"))
            list_ctrl.InsertColumn(4, _("State"))
            list_ctrl.InsertColumn(5, _("Local Endpoint"))
        ...
        # 3. Action Buttons Sizer with Clean Single-Announcement Accelerators
    ...
    def on_tab_changed(self, event):
        """Adjusts column widths without stealing keyboard focus from the tab bar."""
        event.Skip()
    ...
    def _adjust_column_widths(self):
        ...
        for list_ctrl in self.tab_lists:
            ...
            if w <= 0:
                continue
            list_ctrl.SetColumnWidth(0, int(w * 0.18))  # Remote IP
            list_ctrl.SetColumnWidth(1, int(w * 0.24))  # Host / Domain
            list_ctrl.SetColumnWidth(2, int(w * 0.16))  # Port / Service
            list_ctrl.SetColumnWidth(3, int(w * 0.10))  # Protocol
            list_ctrl.SetColumnWidth(4, int(w * 0.14))  # State
            list_ctrl.SetColumnWidth(5, int(w * 0.18))  # Local Endpoint
        ...
        try:
            ...
            for r in tcp_rows:
                if r.dwOwningPid in self.pids:
                    ...
                    key = f"TCP_{l_ip}:{l_port}_{r_ip}:{r_port}"
                    discovered[key] = {
                        "key": key,
                        "proto": "TCP",
                        "remote_ip": r_ip,
    ...
    def _update_connections_in_place(self, active_sockets):
        ...
        Marks closed sockets as 'Closed' and updates tabs smoothly.
        """
    ...
    def _refresh_all_tab_lists(self):
        ...
        # Synchronize each tab's ListCtrl in-place
    ...
    def _sync_single_list(self, list_ctrl, items):
        """Synchronizes items into a specific ListCtrl using Freeze/Thaw to prevent UI flicker."""
        list_ctrl.Freeze()
        try:
            ...
            for idx, item in enumerate(items):
                ...
                if idx < current_rows:
                    list_ctrl.SetItem(idx, 0, item["remote_ip"])
                    list_ctrl.SetItem(idx, 1, host_display)
                    list_ctrl.SetItem(idx, 2, r_port_display)
                    list_ctrl.SetItem(idx, 3, item["proto"])
                    list_ctrl.SetItem(idx, 4, item["state"])
                    list_ctrl.SetItem(idx, 5, item["local_endpoint"])
                else:
                    list_ctrl.InsertItem(idx, item["remote_ip"])
                    list_ctrl.SetItem(idx, 1, host_display)
                    list_ctrl.SetItem(idx, 2, r_port_display)
                    list_ctrl.SetItem(idx, 3, item["proto"])
                    list_ctrl.SetItem(idx, 4, item["state"])
    ...
    def get_selected_connection(self):
        """Retrieves selected connection dictionary from the currently active tab list."""
        ...
        filtered_items = [it for it in self.connection_history.values() if it.get("domain_category") == cat_key]
    ...
    def on_test_port_action(self, event=None, force_copy=False):
        ...
        if not conn or conn["remote_ip"] in ("*", "0.0.0.0"):
            _trigger_tracker_feedback(_("Cannot test local or listening endpoint"), is_success=False)
            return

        # Check if Shift key is held down or explicitly requested via context menu
    ...
    def on_drop_connection(self, event=None):
        ...
        if not conn or conn["proto"] != "TCP" or conn["state"] == _("Closed"):
            _trigger_tracker_feedback(_("Cannot drop this connection"), is_success=False)
            return
        ...
        if success:
            ...
            self._refresh_all_tab_lists()
            _trigger_tracker_feedback(_("Connection severed successfully"), is_success=True)
        else:
            _trigger_tracker_feedback(_("Failed to sever connection (Admin privileges recommended)"), is_success=False)
    ...
    def on_clear_inactive(self, event):
        ...
        self._refresh_all_tab_lists()
        _trigger_tracker_feedback(_("Cleared inactive connections"), is_success=True)
    ...
    def on_open_browser(self, event):
        ...
        if not conn or conn["remote_ip"] in ("*", "0.0.0.0", "127.0.0.1"):
            _trigger_tracker_feedback(_("Cannot open local or internal address"), is_success=False)
            return
        ...
        try:
            os.startfile(url)
            _trigger_tracker_feedback(_("Opening {url}").format(url=url), is_success=True)
        except Exception:
            _trigger_tracker_feedback(_("Failed to open web browser"), is_success=False)
    ...
    def on_check_threat(self, event):
        ...
        if not conn or conn["domain_category"] != "internet":
            _trigger_tracker_feedback(_("Threat lookup is available for public internet IPs only"), is_success=False)
            return
        ...
        try:
            os.startfile(url)
            _trigger_tracker_feedback(_("Checking IP safety on VirusTotal..."), is_success=True)
        except Exception:
            _trigger_tracker_feedback(_("Failed to open threat lookup"), is_success=False)
    ...
    def on_context_menu(self, event):
        ...
        m_test_copy = menu.Append(wx.ID_ANY, _("Copy Port Test &Result"))
...
def show_process_network_tracker_dialog(app_name, pids):
    """Entry point safely presenting the Network Tracker dialog on the main UI thread."""
    if not pids:
        _trigger_tracker_feedback(_("No active PIDs to track for {app}").format(app=app_name), is_success=False)
        return
    ...
    if gui_parent:
        gui_parent.prePopup()
    try:
        dlg = ProcessNetworkTrackerDialog(gui_parent, app_name, pids)
        dlg.ShowModal()
        dlg.Destroy()
    finally:
        if gui_parent:
...
def track_active_app_network():
    """Smart entry point capturing ONLY the PIDs belonging to the active application."""
    focus = api.getFocusObject()
    if not focus:
        _trigger_tracker_feedback(_("No active application found"), is_success=False)
        return

    app_name = getattr(getattr(focus, "appModule", None), "appName", "")
    if not app_name:
        app_name = focus.name[:30] if focus.name else _("Application")
    ...
    matched_pids = set()
    focus_pid = getattr(focus, "processID", None)
    if focus_pid:
        matched_pids.add(focus_pid)

    # Use WTS API to match PIDs of the target application safely
    p_proc_info = ctypes.POINTER(WTS_PROCESS_INFO)()
    proc_count = wintypes.DWORD(0)
    if WTSEnumerateProcesses(WTS_CURRENT_SERVER_HANDLE, 0, 1, ctypes.byref(p_proc_info), ctypes.byref(proc_count)):
        try:
            for i in range(proc_count.value):
                p = p_proc_info[i]
                p_name = (p.pProcessName or "").lower()
                if p_name == target_lower:
                    matched_pids.add(p.ProcessId)
        finally:
            WTSFreeMemory(p_proc_info)

    wx.CallAfter(show_process_network_tracker_dialog, app_name, list(matched_pids))