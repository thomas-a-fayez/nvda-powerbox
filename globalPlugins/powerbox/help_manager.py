...
def show_global_help(plugin_obj):
    ...
    help_lines = [
        ...
        "--- Smart Mouse Routing & Controls ---",
        f"Smart Left Click: {get_current_gesture(plugin_obj, 'leftClick', 'NVDA+Windows+C')}",
        f"Smart Right Click: {get_current_gesture(plugin_obj, 'rightClick', 'NVDA+Windows+X')}",
        f"Smart Double Click: {get_current_gesture(plugin_obj, 'doubleClick', 'NVDA+Windows+Z')}",
